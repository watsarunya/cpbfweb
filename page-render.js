  ...
  async function init(container) {
    ...
    if (!page) {
      ...
      if (attemptedLookup && container.dataset.additive !== 'true') {
        // path relative (ไม่ใช่ '/404.html' แบบ absolute) เพราะ absolute path จะ resolve ผิดตอนเปิดผ่าน
        // file:// ตรงๆ (ไปที่ root ของ filesystem แทนที่จะเป็น root ของโปรเจกต์ — ได้ ERR_FILE_NOT_FOUND)
        // ทุกเพจที่ใช้ page-render.js อยู่ที่ root ของโปรเจกต์เดียวกับ 404.html เสมอ path relative นี้จึง
        // ใช้ได้ถูกต้องทั้งตอนเปิดผ่าน file:// และตอน deploy จริงบน Netlify (absolute path ก็ทำงานถูกต้อง
        // บน Netlify เช่นกัน แต่ relative ทำงานถูกต้องในทั้งสองสภาพแวดล้อม)
