...
> `<iframe>`/class/data-* attribute/inline style อื่นนอกจาก color **หายหมดทันที** เหลือแค่ `<p>`/`<span>`
> เปล่าๆ (เช่น `<div class="x"><table>...</table></div>` → เหลือแค่ `<p>...</p>` ธรรมดา) — ตัว handler
> `quill.on('text-change', ...)` เดิม sync `draft[key]` จาก `quill.root.innerHTML` ทุกครั้งไม่ว่าที่มาไหน
> ทำให้โค้ดที่ผู้ใช้เพิ่งพิมพ์ไว้ถูกต้อง (เก็บอยู่ใน `draft[key]` แล้วจาก source-mode's input handler) โดน
> **ทับด้วยเวอร์ชันที่ถูกตัดทิ้งไปแล้ว**ทันทีที่ `dangerouslyPasteHTML()` ทำงาน — **แก้โดยเช็ค `source`
> parameter ของ `text-change` event** (Quill ส่ง `'user'` เมื่อพิมพ์/กดปุ่ม toolbar เอง vs `'api'` เมื่อเรียก
> `dangerouslyPasteHTML()` — ยืนยันด้วย browser test ตรงๆ) sync `draft[key]` เฉพาะตอน `source==='user'`
> เท่านั้น และให้ตอนกดปุ่มกลับ WYSIWYG ตั้ง `draft[key] = sourceTextarea.value` ตรงๆ ก่อนเสมอ (ค่าที่จะถูก
> save จริง ไม่ผ่าน Quill เลย) ส่วน `dangerouslyPasteHTML()` ยังเรียกอยู่แค่เพื่อให้กล่อง WYSIWYG แสดงตัวอย่าง
> เท่าที่ Quill พอทำได้เฉยๆ (`source='api'` จาก text-change จะไม่ทับ draft ซ้ำแล้ว) — **trade-off ที่ยอมรับ**:
> กล่อง WYSIWYG อาจแสดงผลไม่ตรงกับที่ save จริง 100% ถ้าโค้ดมี tag/attribute ที่ Quill ไม่รู้จัก (live preview
> ด้านล่างซึ่งไม่ผ่าน Quill เลยจะถูกต้องเสมอ) และถ้าผู้ใช้พิมพ์ต่อใน WYSIWYG ต่อจากนั้นจริง (source='user')
> จะ sync ทับด้วยเวอร์ชันที่ Quill เข้าใจแทนตามปกติของ editor แบบผสม (คาดหวังได้อยู่แล้ว) — ทดสอบผ่าน browser
> จริงยืนยันแล้วว่า พิมพ์ `<div class="promo-box" style="..."><table>...</table><p>plain text</p></div>` ผ่าน
> source mode แล้วกลับ WYSIWYG → `draft[key]` ตรงกับโค้ดต้นฉบับ**เป๊ะทุกตัวอักษร** แม้กล่อง WYSIWYG จะแสดงแค่
> `<p>A</p><p>B</p><p>plain text</p>` (เวอร์ชันที่ Quill ตัดทิ้งไปแล้ว) ก็ตาม — ดูหัวข้อ "Rich Text Editor
> กลาง (Quill)" ด้านล่าง
>
> อัปเดตก่อนหน้าในวันเดียวกัน (2026-08-06): **`cms/page-editor.html`'s rich text editor (เนื้อหา section) เพิ่มปุ่ม "</>"
> แก้โค้ด HTML ดิบได้โดยตรงแล้ว** (ผู้ใช้ขอเฉพาะจุดนี้ — ไม่ได้แตะ Quill editor ของสินค้า/บทความ) — เดิมถ้าไม่ใช้
> layout `custom-html` (ซึ่งมีช่องโค้ด HTML ดิบอยู่แล้วแยกต่างหาก) แอดมินแก้เนื้อหา section แบบปกติ
> (image-left/right/top/bottom) ได้ผ่าน Quill WYSIWYG เท่านั้น ไม่มีทางแก้โค้ด HTML ตรงๆ เลย — เพิ่มปุ่ม
> "</>" ไว้มุมขวาบนของ label "เนื้อหา (TH)"/"เนื้อหา (EN)" แต่ละอัน กดแล้วสลับ `.cms-rich-editor` (ตัว Quill
> ทั้งก้อน toolbar+editor) เป็น `<textarea class="cms-rich-editor__source">` (font monospace) แสดงโค้ด HTML
> ดิบปัจจุบัน แก้ไขได้ตรงๆ พิมพ์แล้ว sync เข้า `draft[key]` + refresh live preview สดๆ เหมือน WYSIWYG ปกติ —
> กดปุ่มซ้ำเพื่อกลับไป WYSIWYG จะ parse โค้ดที่แก้กลับเข้า Quill ผ่าน `quill.clipboard.dangerouslyPasteHTML()`
> (แทนที่เนื้อหาเดิมทั้งหมด) ซึ่ง trigger `text-change` ให้เองอัตโนมัติ ไม่ต้อง sync `draft[key]` ซ้ำมือหลังกลับ
> — CSS ใหม่ `.cms-rich-editor__label-row`/`__source-toggle`/`.cms-rich-editor__source` ใน `cms/style.css` —
> ทดสอบผ่าน browser จริงครบวงจร (inject markup+Quill จริงเข้าไปในหน้า `cms/page-editor.html` ที่โหลดอยู่ ไม่ได้
> login เพราะไม่มี credential): กด "</>"  → เห็นโค้ด HTML ดิบถูกต้อง (`<p>สวัสดี <strong>ทดสอบ</strong></p>`),
> แก้โค้ดเพิ่ม `<em>`/`<ul><li>` แล้วกดกลับ WYSIWYG → Quill render ตัวเอียง+bullet list ถูกต้อง และ
> `draft[key]`/`quill.root.innerHTML` sync ตรงกันอัตโนมัติโดยไม่ต้องเขียน sync logic เพิ่มเลย
>
> อัปเดตก่อนหน้าในวันเดียวกัน (2026-08-06): **แก้บั๊กสำคัญ: hamburger menu (มือถือ) กดแล้วเมนูไม่เปิดเลยใน 14 หน้า** — ผู้ใช้
> รายงานว่า "หน้าเมนูย่อยอื่นๆ" กดปุ่ม hamburger แล้วเมนูไม่แสดง — ตรวจสอบพบว่าปุ่ม `#headerHamburger` มี markup
> อยู่ครบทุกหน้า แต่ **ไม่มี event listener ผูกไว้เลย** ใน 14 จาก 26 หน้า HTML ที่ root: `404.html`,
> `arabitia.html`, `beans_ingredients.html`, `beverage_ingredients.html`, `catering.html`, `coffee_shop.html`,
> `coffee_shop_equipment.html`, `fuji_premium_water.html`, `jungle.html`, `lumi.html`, `oem_beans.html`,
> `oem_water.html`, `promo.html`, `snack_box.html`, `vending.html` — grep หา `hamburger.addEventListener`
> ยืนยันว่าไม่มีเลยสักบรรทัดในไฟล์เหล่านี้ ทั้งที่หน้าอื่น (เช่น `our_story.html`/`index.html`) มี `<script>`
> บล็อก "Mobile nav drawer + accordion dropdown toggle" วางไว้ทันทีหลัง `</footer>` ครบถ้วน — พบว่าทุกไฟล์ที่
> เสียมี **ช่องว่างตรง `</footer>`** พอดี (บางไฟล์เหลือบรรทัดว่าง 1-2 บรรทัด บางไฟล์ติดกันเลยไม่มีช่องว่างด้วยซ้ำ)
> ตรงตำแหน่งที่ควรมี `<script>` บล็อกนี้อยู่ — สันนิษฐานว่าโดนลบไปพร้อมกับตอนย้ายเนื้อหาเดิมออกจาก 16 หน้าเข้า
> Page Management (ดูหัวข้อ "สิ่งที่ต้องทำต่อ" ข้อ 2 เรื่อง "เนื้อหาเดิมถูกลบออกจากไฟล์ HTML แล้ว") น่าจะเป็น
> collateral damage ตอนนั้น — **แก้โดย copy บล็อก `<script>` ทั้ง 2 ก้อนจาก `our_story.html`** (ก้อนแรก
> "Mobile nav drawer + accordion dropdown toggle" ผูก hamburger click + accordion dropdown บนมือถือ, ก้อนที่
> สอง "Header: ลด bg เป็น transparent 70% ตอน scroll" ผูก `.site-header.is-scrolled` — **หายไปคู่กันทั้ง 2
> ก้อนในทุกไฟล์เดียวกัน** เลยแก้พร้อมกันในรอบนี้ด้วย แม้ผู้ใช้จะถามแค่เรื่อง hamburger) แปะกลับเข้าไปทันทีหลัง
> `</footer>` ของทั้ง 14 ไฟล์ ยืนยันด้วยสคริปต์ตรวจสอบว่าเนื้อหาที่แปะเข้าไป**ตรงกับต้นฉบับ (`our_story.html`)
> เป๊ะทุกตัวอักษร** ไม่ใช่แค่ "หน้าตาคล้ายกัน" — และนับ `<script>`/`</script>` คู่กันครบทุกไฟล์ (ไม่มีแท็กเหลือ/
> ขาด) — ทดสอบผ่าน browser จริงที่ viewport มือถือ (375px): เดิมกด/`.click()` โปรแกรมบนปุ่ม hamburger ของ
> `beans_ingredients.html` แล้ว class/`aria-expanded` ไม่เปลี่ยนเลย (พิสูจน์ว่า listener ไม่เคยผูกจริง ไม่ใช่
> แค่ปัญหา CSS/coordinate) หลังแก้แล้วทดสอบซ้ำบน `oem_beans.html` (หน้าที่ไม่เคยเปิดมาก่อนในรอบทดสอบนี้) ยืนยัน
> ว่า class/`aria-expanded` เปลี่ยนถูกต้อง และเมนู slide-in แสดงผลเต็มจอถูกต้อง (screenshot ยืนยันแล้ว) —
> ⚠️ **หน้าที่เหลือ 12 หน้า** (`career.html`/`cart.html`/`contact.html`/`fix_repair.html`/`index.html`/
> `news-detail.html`/`newsroom.html`/`online_shop.html`/`our_service.html`/`our_story.html`/
> `product-detail.html`) **ไม่ได้แตะ เพราะมี binding อยู่ถูกต้องอยู่แล้ว** (ตรวจสอบด้วย grep ก่อนแก้ ไม่ใช่
> แค่เดา) — ⚠️ ถ้าเพิ่มหน้าใหม่ในอนาคตโดย copy จากหน้าที่เคยเสีย ต้องเช็คว่ามี `<script>` 2 บล็อกนี้ครบก่อนเสมอ
>
> อัปเดตก่อนหน้าในวันเดียวกัน (2026-08-06): **แก้บั๊กสำคัญ: KV Banner ล้นขวางจอทั้งหน้าบนมือถือ/แท็บเล็ต
> (จอแคบกว่า 1440px)** — ผู้ใช้รายงานว่า KV Banner "ไม่ responsive" หลังจากรัน `schema-banners-v4.sql`/`seed-newsroom-page.sql`
> แล้วเห็น KV Banner จริงบนเว็บเป็นครั้งแรก (ก่อนหน้านี้ไม่เคย render เลยเพราะ migration ค้างอยู่) —
> ตรวจสอบพบว่าเป็นบั๊กที่เพิ่งเกิดจากการเพิ่ม `aspect-ratio:1440/360` ให้ `.kv-banner` เมื่อ 2026-08-04 (งาน
> ปรับขนาดแนะนำ Hero/KV Banner) — **สาเหตุ**: `.kv-banner` เดิมไม่มี `width` ระบุไว้ตรงๆ เลย (ต่างจาก
> `.hero-showcase` ที่มี `width:100%` อยู่แล้วตั้งแต่แรก) บนจอแคบกว่า 1440px ที่ `aspect-ratio` คำนวณความสูง
> จากความกว้าง container แล้วได้ค่าน้อยกว่า `min-height:360px` (เช่นจอมือถือ 375px → สูงแค่ ~94px) ทำให้
> `min-height` ต้อง "ชนะ" กลายเป็นตัวกำหนดความสูงแทน — แต่เพราะ `width` เป็น `auto` (ไม่ได้ระบุชัดว่า 100%)
> browser เลย **คำนวณ width ใหม่จาก min-height × aspect-ratio แทนที่จะยึดความกว้างของ container เดิม**
> (375px → กลายเป็น 360×4 = 1440px จริงๆ ตามสเปก CSS aspect-ratio) ทำให้ section กว้างล้นจอมือถือทั้งหน้าไป
> เลย (ยืนยันจริงผ่าน browser: `.kv-banner`'s computed `width` = 1440px ทั้งที่ parent container กว้างแค่
> 375px, `window.innerWidth` เพี้ยนตามไปเป็น 1440 ทั้งหน้าด้วย เพราะทั้งหน้าต้อง layout ให้พอดีกับ section ที่
> กว้างเกิน) — **แก้โดยเพิ่ม `width:100%` ให้ `.kv-banner` ตรงๆ** (pattern เดียวกับ `.hero-showcase`) บังคับ
> width ให้เป็น 100% ของ container เสมอไม่ว่า aspect-ratio/min-height จะคำนวณออกมาเท่าไหร่ก็ตาม (สัดส่วน 4:1
> จะไม่คงอยู่แม่นยำ 100% ตอนที่ `min-height` ชนะบนจอเล็ก — ยอมรับเป็น trade-off ดีกว่า section ล้นจอ) —
> ทดสอบผ่าน browser จริงยืนยันครบ 3 breakpoint: มือถือ (375px) `.kv-banner` width ถูกต้องเป็น 375px (ไม่ใช่
> 1440px แบบเดิม) ไม่มี horizontal overflow เลยทั้งหน้า (`scrollWidth === clientWidth`), แท็บเล็ต (768px) ถูกต้อง,
> เดสก์ท็อป (1440px) ยังคง 1440×360 ถูกต้องเหมือนเดิมไม่กระทบ — ดูหัวข้อ "ระบบ Banner" ด้านล่าง
>
> อัปเดตก่อนหน้า (2026-08-04): **แก้บั๊ก: เปลี่ยนภาษาเว็บไซต์แล้ว Hero Banner (และ KV Banner) ไม่เปลี่ยนรูปตาม
> ภาษา** — สาเหตุ: `banner-render.js`'s `buildHeroSlide()`/`buildKvSection()` ตั้ง `data-th`/`data-en` (ไม่มี
> suffix) บน `<img>`/`<video>` เพื่อเก็บ URL รูป TH/EN ไว้สลับตอนเปลี่ยนภาษา แต่ `data-th`/`data-en` เฉยๆ เป็น
> convention ของ `i18n.js`'s `applyTextSwap()` สำหรับสลับ **`.textContent`** เท่านั้น (ใช้กับ heading/ปุ่ม/
> ข้อความทั่วไป) ตั้งค่านี้บน `<img>`/`<video>` แล้วไม่มีผลอะไรเลยเพราะ textContent ของ media element ไม่ได้
> render เป็นภาพ — ทำให้ `src` ของ Hero/KV Banner ค้างอยู่ที่ค่าตอนโหลดหน้าครั้งแรกตลอด ไม่เคยสลับตามภาษาที่
> เลือกไว้เลย (เทียบกับ `nav-render.js` ที่ทำถูกอยู่แล้ว ใช้ `data-th-alt`/`data-en-alt` สำหรับสลับ `alt`
> attribute โดยเฉพาะ ไม่ชนกับ convention ของ textContent) — แก้โดยเปลี่ยนเป็น `data-th-src`/`data-en-src`
> (ตรงกับ pattern `-alt`/`-title`/`-placeholder`/`-aria-label` ที่มีอยู่แล้ว) พร้อมเพิ่ม handler ใหม่ใน
> `i18n.js`'s `applyLang()` สลับ attribute `src` โดยเฉพาะเมื่อเจอ `data-en-src`/`data-th-src` — ทดสอบผ่าน
> browser จริงแล้ว (คลิกตัวสลับภาษาจริงบนหน้าเว็บ ไม่ใช่เรียกฟังก์ชันตรงๆ): ใส่ค่าทดสอบแยกกันใน
> `data-th-src`/`data-en-src` ของ Hero Banner บน `index.html` แล้วคลิกสลับเป็น EN จริง ยืนยันว่า `src`
> เปลี่ยนค่าตามถูกต้อง — **ไม่ได้ทดสอบ KV Banner จริงบนเว็บ** เพราะ KV banner ของ `index.html` ที่มีอยู่แล้ว
> (ตั้งแต่ก่อน session นี้) ยังไม่ render บนเว็บเลยตอนนี้ ด้วยเหตุผลคนละเรื่อง (ค้างมาจาก `schema-banners-
> v4.sql` ที่ยังไม่ได้รัน ดู "สิ่งที่ต้องทำต่อ" ข้อ 0e) แต่ KV Banner ใช้ mechanism เดียวกันเป๊ะกับ Hero
> Banner (attribute เดียวกัน, handler เดียวกันใน i18n.js) จึงควรถูกต้องเช่นกันเมื่อ migration นั้นรันแล้ว
>
> อัปเดตก่อนหน้าในวันเดียวกัน (2026-08-04): **Hero Banner + KV Banner ทุกหน้า รองรับ `.gif`/`.mp4` แล้ว** (ผู้ใช้ขอ ให้ใช้
> วิดีโอพื้นหลังได้) — `.gif` รองรับอยู่แล้วเดิม งานหลักคือเพิ่ม `.mp4`: `cms/upload.js`'s `cmsUploadImage()`
> รับ `opts.allowVideo` ใหม่ (จำกัดไว้เฉพาะ Hero/KV Banner เท่านั้น ไม่ได้เปิดให้จุดอื่นอัปโหลดวิดีโอด้วย) ลิมิต
> ขนาดวิดีโอ 20MB (สูงกว่ารูปภาพปกติ 5MB) — dropzone TH/EN ใน `cms/banners.html` เพิ่ม `<video class=
> "cms-dropzone__preview-video">` คู่กับ `<img>` เดิม toggle กันตามนามสกุลไฟล์ (ใช้ `el.style.display` ตรงๆ
> แทน `.hidden` กัน bug specificity แบบเดิมที่เจอมาหลายรอบ) — ฝั่งเว็บหลัก `banner-render.js`'s
> `buildHeroSlide()`/`buildKvSection()` สร้าง `<video autoplay muted loop playsinline>` แทน `<img>` เมื่อเจอ
> `.mp4` โดยใช้ class เดิมเป๊ะๆ (`hero-showcase__bg-image`/`kv-banner__image`) ไม่ต้องเพิ่ม CSS ใหม่เลยเพราะ
> selector เดิมเป็น generic ไม่ผูกกับ tag `img` โดยเฉพาะ — CMS preview modal (เดิมใช้ CSS background-image
> ล้วนๆ ซึ่งไม่รองรับวิดีโอเลย) เพิ่ม `buildPreviewVideoEl()` ใหม่ด้วย — ตรวจสอบ Supabase Storage bucket แล้ว
> ไม่มี MIME/size restriction ระดับ bucket เลย ไม่ต้องรัน SQL migration เพิ่ม — ดูหัวข้อ "ระบบ Banner" ด้านล่าง
>
> อัปเดตก่อนหน้าในวันเดียวกัน (2026-08-04): **newsroom.html รองรับ KV Banner ได้แล้ว** (ผู้ใช้ขอ) — เดิม `contact.html`/
> `newsroom.html` ถูกกันออกจาก Page Management ทั้งคู่โดยตั้งใจตั้งแต่แรก (มีฟอร์ม/เป็นหน้า listing แบบ dynamic
> อยู่แล้ว) แต่ KV Banner render ผ่าน "static proxy" `page_sections` mechanism เท่านั้น ต้องมี
> `#pageSectionsContainer`+`page-render.js` ถึงจะมีที่ให้ render ลงไปได้ — newsroom.html ที่ไม่เคยมี Page
> Management เลยเพิ่ม KV Banner ไม่ได้จริงเลย ทั้งที่หน้า "จัดการแบนเนอร์" ไม่เคยบล็อกไม่ให้เลือกหน้านี้ (พบ
> เพิ่มว่าแอดมินเคยเพิ่ม KV banner ให้หน้านี้ไว้แล้วจริงหนึ่งอัน แต่ไม่เคยแสดงผลได้เลยตั้งแต่สร้าง เพราะหา
> `pages` row ของ newsroom ไม่เจอ) — แก้โดยเพิ่ม newsroom.html เข้า Page Management แบบ **เสริม (additive)**
> เหมือน `online_shop.html` เป๊ะๆ (เพิ่ม `page-render.js`+DOMPurify+`#pageSectionsContainer
> data-additive="true"` ไม่แตะเนื้อหาจริงของหน้าเลย) พร้อมไฟล์ SQL ใหม่ `cms/seed-newsroom-page.sql`
> (**ยังไม่ได้รัน**) สร้างแถว `pages` ให้ + backfill proxy section ให้ KV banner ที่มีอยู่แล้วนั้นในตัวเดียว —
> ดูหัวข้อ "หน้า newsroom.html" ใต้ "ระบบจัดการเพจ" ด้านล่างสำหรับรายละเอียดเต็ม
>
> อัปเดตก่อนหน้าในวันเดียวกัน (2026-08-04): **สินค้า/บทความที่ปิดใช้งาน (`is_active=false`) เข้าถึงผ่าน URL ตรงไม่ได้แล้ว
> แสดง 404 แทน** (ผู้ใช้ขอ) — เดิม `product-detail.html`/`news-detail.html` (route ผ่าน `?id=<uuid>`) query
> ด้วย id ตรงๆ อย่างเดียว ไม่เคยเช็ค `is_active` เลย ทำให้เปิดสินค้า/บทความที่แอดมินปิดใช้งานไว้ผ่านลิงก์ตรง
> ได้ปกติเงียบๆ (หน้า list/carousel ทุกจุด เช่น `online_shop.html`/`newsroom.html`/`index.html` กรอง
> `is_active=true` อยู่แล้วถูกต้องมาตลอด — ปัญหามีแค่จุดเข้าถึงตรงผ่าน id) — แก้โดยเพิ่ม `.eq('is_active',
> true)` ใน `products-render.js`'s `fetchProductById()` และ `news-render.js`'s `fetchArticleById()`
> (pattern เดียวกับที่ `fetchProducts()`/`fetchArticles()`/`fetchCategories()` ใช้อยู่แล้วในไฟล์เดียวกัน)
> แล้วให้ทั้ง 2 หน้า **redirect ไป `404.html`** (path relative ตาม pattern ที่แก้ไว้ก่อนหน้านี้ในวันเดียวกัน
> — ดูด้านล่าง) เมื่อ `fetchProductById()`/`fetchArticleById()` คืน `null` แทนที่จะโชว์ข้อความ "ไม่พบสินค้า/
> บทความนี้" แบบเดิมที่ยังอยู่ในหน้าเดิมเฉยๆ ครอบคลุมทั้ง 3 เคสด้วยเงื่อนไขเดียว (ไม่มี `?id=`, id ผิด/ไม่มี
> อยู่จริง, ปิดใช้งานอยู่) — ฝั่ง CMS (`cms/products.js`/`cms/news-articles.js`) ไม่กระทบเลย เพราะโหลด/แก้ไข
> รายการจาก `items` array ที่ query แยกต่างหาก ไม่ผ่านฟังก์ชันที่แก้ (แอดมินยังเปิด/แก้ไข/เปิดใช้งานรายการที่
> ปิดอยู่ได้ตามปกติ) — ยืนยัน `.eq('is_active', true)` ไม่มีจุดอื่นเรียกใช้ `fetchProductById()`/
> `fetchArticleById()` นอกจาก 2 หน้านี้ด้วย grep ทั้งโปรเจกต์แล้ว ดูหัวข้อ "ระบบจัดการสินค้า"/"ระบบข่าวสาร"
> ด้านล่างสำหรับรายละเอียดเต็ม
>
> อัปเดตก่อนหน้าในวันเดียวกัน (2026-08-04): **แก้บั๊กสำคัญ: สร้างเพจ standalone ใหม่ไม่ได้เลยสักอัน (ขึ้น error ว่า
> slug ชนไฟล์จริงทั้งที่ไม่ได้ชน)** — ผู้ใช้รายงานว่าสร้างเพจ slug "test4" (ไม่มี `test4.html` อยู่จริงเลย)
> แล้วโดน guard บล็อกว่า "ชนกับไฟล์ test4.html ที่มีอยู่แล้ว" สาเหตุ: guard เดิมใน `cms/pages.js`'s
> `checkFileExists()` เช็คด้วย `fetch(HEAD)` จริงไปที่ root แล้วดู `res.ok` — ใช้ได้ตอนยังไม่ deploy
> `_redirects` (ไฟล์ไม่มีจริงจะได้ 404 → `res.ok=false` ถูกต้อง) แต่**พังทันทีหลัง deploy `_redirects`**
> เพราะกฎ rewrite `/*.html /promo.html?slug=:splat 200` จับ path `.html` ที่ไม่มีไฟล์จริงตรงกัน**ทุกอัน**
> แล้วตอบสถานะ **200** เสมอ (rewrite ไป `promo.html` สำเร็จ ไม่ใช่ 404) — ทำให้ `fetch` แยกแยะ "ไฟล์จริง"
> กับ "ไม่มีไฟล์เลย" ผ่าน HTTP status ไม่ได้อีกต่อไป ทุก slug จะดูเหมือนชนไฟล์จริงตลอดไป บล็อกการสร้างเพจ
> standalone ใหม่แบบถาวร — **แก้โดยเลิกใช้ fetch เช็คไปเลย** เปลี่ยนเป็นเช็คกับลิสต์คงที่ `KNOWN_HTML_FILES`
> (array ชื่อไฟล์ `.html` จริง 27 ไฟล์ที่ root ของโปรเจกต์ ซิงค์กับผลลัพธ์จริงของ `find` ณ ตอนแก้)
> `checkFileExists()` กลายเป็นฟังก์ชัน sync ธรรมดา ไม่พึ่งเครือข่ายเลย ทำงานถูกต้องไม่ว่า `_redirects` จะ
> deploy แล้วหรือยัง — ทดสอบผ่าน `deno run` ยืนยันแล้วว่า slug "career" (ชนไฟล์จริง) → block ถูกต้อง,
> slug "test4"/"summer-sale-2026" (ไม่ชน) → ไม่ block ถูกต้อง (เคสที่บั๊กเดิมรายงานผิด) — ⚠️ **ต้องอัปเดต
> `KNOWN_HTML_FILES` ในนี้ทุกครั้งที่เพิ่ม/ลบ/เปลี่ยนชื่อไฟล์ `.html` จริงที่ root** (จุดที่ 3 ที่ต้องซิงค์
> ต่อจาก href ใน static HTML + `url` column ใน `menu_items`) — **`cms/menu.js`'s `checkFileExists()`
> (คนละจุด ใช้ตอน auto-สร้างเพจให้เมนูใหม่) ยังเป็น `fetch(HEAD)` แบบเดิม ไม่ได้แก้รอบนี้** เพราะผลกระทบ
> เบากว่า (แค่ไม่ swap URL เมนูไปเป็น `promo.html?slug=...` ให้ — แต่กฎ rewrite ใน `_redirects` จับ URL
> ที่ไม่มีไฟล์จริงให้เองอยู่ดี ไม่ทำให้เมนูเปิดไม่ได้เหมือนที่ `cms/pages.js` เจอ — ยืนยันด้วยการตรวจโค้ด
> เท่านั้น ไม่ได้ทดสอบ end-to-end) ดูหัวข้อ "เพจ standalone (URL สะอาด)" ใต้ "ระบบจัดการเพจ" ด้านล่าง
>
> อัปเดตก่อนหน้าในวันเดียวกัน (2026-08-04): **แก้บั๊กที่สอง (พบระหว่างตรวจบั๊กแรกข้างบน): เข้าถึงเมนู/เพจที่
> `is_active=false` ผ่าน URL ตรง (หรือทดสอบผ่าน `file://` เปิดไฟล์ตรงๆ) แล้วขึ้น `ERR_FILE_NOT_FOUND`/
> "Your file couldn't be accessed" แทนที่จะเห็นหน้า 404 ของเว็บเอง** — สาเหตุ: `page-render.js`'s `init()`
> (บรรทัดที่ redirect ไปหน้า 404 อัตโนมัติเมื่อ `isPageVisible()` คืน `false` — ดูหัวข้อ "หน้า 404" ใต้
> "ระบบจัดการเพจ") เรียก `window.location.replace('/404.html')` ด้วย **absolute path** (ขึ้นต้นด้วย `/`) —
> ตอน deploy จริงบน Netlify path นี้ resolve ถูกต้อง (`/404.html` = root ของโดเมน) แต่ตอนเปิดผ่าน `file://`
> ตรงๆ (เช่น double-click ไฟล์ `.html` ทดสอบเอง ไม่ผ่าน HTTP server) `/404.html` จะ resolve ไปที่ **root ของ
> filesystem เครื่อง** (`file:///404.html`) แทนที่จะเป็น root ของโปรเจกต์ — ไฟล์นั้นไม่มีอยู่จริงเลยจึงขึ้น
> `ERR_FILE_NOT_FOUND` ของ browser เอง (ตรงกับ screenshot ที่ผู้ใช้ส่งมา: address bar ขึ้น "File /404.html")
> — แก้โดยเปลี่ยนเป็น **relative path** `'404.html'` (ไม่มี `/` นำหน้า) แทน — ใช้ได้ถูกต้องทั้ง 2 สภาพแวดล้อม
> เพราะทุกเพจที่ใช้ `page-render.js` อยู่ที่ root ของโปรเจกต์เดียวกับ `404.html` เสมอ (ไม่มีเพจไหนอยู่ใน
> subfolder) — ทดสอบยืนยันด้วย `new URL()` จริงในเบราว์เซอร์ (ไม่ใช่แค่ทฤษฎี): จาก
> `file:///Users/.../Website B&F/index.html`, path เดิม `/404.html` resolve เป็น `file:///404.html`
> (ตรงกับบั๊กที่รายงาน) ส่วน path ใหม่ `404.html` resolve เป็น `file:///Users/.../Website B&F/404.html`
> ซึ่งเป็นไฟล์จริงที่มีอยู่ (ยืนยันด้วย `ls` แล้ว) — grep ทั้งโปรเจกต์แล้วยืนยันว่านี่เป็นจุดเดียวที่ใช้
> `/404.html` แบบ absolute (ไม่มีจุดอื่นต้องแก้เพิ่ม)
>
> อัปเดตก่อนหน้า (2026-08-01): **KV Banner หลายอันในเพจเดียวกันแยกเป็นคนละ section จัดการตำแหน่งอิสระต่อกัน
...
> ⚠️ อัปเดต 2026-08-04 — **Hero Banner + KV Banner ทุกหน้า รองรับไฟล์ `.gif`/`.mp4` แล้ว** (นอกจาก jpg/jpeg/
> png เดิม) ผู้ใช้ขอเพื่อให้ใช้วิดีโอพื้นหลังได้ — `.gif` รองรับอยู่แล้วเดิมจริงๆ (`cmsUploadImage()`'s
> `ALLOWED_TYPES` มี `image/gif` อยู่แล้วตั้งแต่แรก) งานหลักคือเพิ่ม `.mp4`:
> - **`cms/upload.js`**: `cmsUploadImage(file, opts)` รับ `opts.allowVideo` ใหม่ — ถ้า true จะขยาย allowed
>   MIME types ให้รวม `video/mp4` และใช้ลิมิตขนาดแยกสำหรับวิดีโอ (`MAX_VIDEO_SIZE_MB = 20`, สูงกว่ารูปภาพ
>   ปกติที่ยังคง 5MB เดิม เพราะไฟล์วิดีโอเล็กสุดก็มักใหญ่กว่ารูปหลาย MB) — **จำกัดไว้เฉพาะจุดที่ส่ง
>   `allowVideo: true` เข้ามาเท่านั้น** (แค่ Hero/KV Banner — ไม่ได้เปิดให้เมนู/สินค้า/บทความ/พื้นหลัง section
>   อัปโหลดวิดีโอด้วย เพราะผู้ใช้ขอเจาะจงแค่ Hero/KV) — `cmsBindImageUpload()` รับ `opts.allowVideo` ส่งต่อให้
>   `cmsUploadImage()` และจัดการพรีวิว: dropzone ต้องมี `<video class="cms-dropzone__preview-video">` เพิ่ม
>   เข้ามาคู่กับ `<img class="cms-dropzone__preview">` เดิม (มีแค่ใน `cms/banners.html`'s Image TH/EN 2 จุด
>   เท่านั้น) toggle ไหนแสดงตามนามสกุลไฟล์ของ URL ที่กรอกไว้ (`isVideoUrl()` เช็ค `.mp4` ท้าย string) — ⚠️
>   **จงใจใช้ `el.style.display = 'block'/'none'` ตรงๆ แทน `.hidden` attribute** เพื่อกัน bug specificity
>   แบบเดียวกับที่เจอมาแล้วหลายรอบ (`.cms-dropzone.has-image .cms-dropzone__preview` ใน CSS มี specificity
>   สูงกว่า `[hidden]` ของ browser)
> - **`banner-render.js`** (ฝั่งเว็บหลัก): `buildHeroSlide()`/`buildKvSection()` เดิมสร้าง `<img>` ตรงๆ เสมอ
>   เปลี่ยนเป็นเรียก `buildMediaEl()` ใหม่ที่เช็ค `isVideoUrl()` ก่อน แล้วสร้าง `<video autoplay muted loop
>   playsinline>` แทนถ้าเป็น `.mp4` — **ใช้ class เดิมเป๊ะๆ** (`hero-showcase__bg-image`/`kv-banner__image`)
>   ไม่ต้องเพิ่ม CSS ใหม่เลยฝั่งเว็บหลัก เพราะ selector เดิมใน `style.css` เป็น generic class selector
>   (`position:absolute; object-fit:cover` ฯลฯ) ไม่ได้ผูกกับ tag `img` โดยเฉพาะ ใช้กับ `video` ได้ทันที
> - **`cms/banners.js`**: preview modal (ปุ่ม "👁️ ดูตัวอย่าง" ที่การ์ดแต่ละใบ) เดิมใช้ CSS `background-image`
>   ล้วนๆ ซึ่ง**ไม่รองรับวิดีโอเลย** (จะไม่เห็นอะไรขึ้นถ้าเลือกไฟล์ mp4) — เพิ่ม `buildPreviewVideoEl()` สร้าง
>   `<video>` จริงแทนเมื่อ `isVideoUrl()` เป็นจริง — `updateImageSizeHint()`/hint ข้อความใต้ dropzone (ทั้ง static
>   fallback ใน `cms/banners.html` และ dynamic ที่ตั้งจาก JS) อัปเดตข้อความเป็น "รองรับไฟล์ jpg, jpeg, png, gif,
>   mp4 ขนาด ... ไม่เกิน 5 MB (วิดีโอไม่เกิน 20 MB)" — `fieldImageThFile`/`fieldImageEnFile`'s `accept` attribute
>   เปลี่ยนจาก `image/*` เป็น `image/*,video/mp4` ด้วย (ไม่งั้น browser's file picker dialog จะกรอง .mp4 ออกไป
>   เลยไม่ให้เลือกตั้งแต่แรก)
> - **Supabase Storage bucket `cms-uploads`**: ตรวจสอบ `cms/schema-storage.sql` แล้วไม่มี `allowed_mime_types`/
>   `file_size_limit` ผูกไว้ที่ bucket level เลย (แค่ RLS policy ต่อ role) — ไม่ต้องรัน SQL migration ใหม่ใดๆ
>   เพิ่มสำหรับรอบนี้ ฝั่ง client-side validation ใน `cmsUploadImage()` เป็นด่านเดียวที่คุมชนิด/ขนาดไฟล์
> - **ทดสอบ**: ไม่มี session login จริงให้ทดสอบอัปโหลดไฟล์จริงขึ้น Supabase (ไม่มี credential) — ยืนยันผ่าน
>   browser จริงด้วยการ inject markup + จำลอง `updatePreview()` logic ตรงๆ ใน `cms/banners.html` ที่โหลดอยู่
>   (force-reload stylesheet คนละก้อนกับ cache เดิมในแท็บก่อน กันปัญหาเดียวกับที่เจอตอนทำ dropzone redesign
>   รอบก่อน): ยืนยันว่าใส่ URL ลงท้าย `.mp4` แล้ว dropzone สลับไปโชว์ `<video>` (ซ่อน `<img>`) ถูกต้อง, class
>   `has-image` ติดถูกต้อง, และ hint ข้อความของอีกช่อง (Image EN ที่ไม่ได้แตะ) แสดงข้อความใหม่ถูกต้องตามที่ตั้งใจ
>   — **ไม่ได้ทดสอบว่าวิดีโอเล่นจริงได้ (autoplay/loop) เพราะ URL ทดสอบเป็นโดเมนภายนอกที่เข้าถึงไม่ได้จาก
>   sandbox นี้** เป็นการยืนยัน layout/logic เท่านั้น ไม่ใช่ end-to-end video playback
...
- **หน้าเว็บหลักที่ต่อสายแล้ว**:
  ...
  - `product-detail.html` — route จริงผ่าน `?id=<uuid>` (เดิม hardcode เป็นสินค้าเดียวตลอด) แสดง
    ...
    **สุ่ม 5 รายการ** (ไม่จำกัดหมวดหมู่แล้ว) — ⚠️🔧 **แก้บั๊ก 2026-08-04**: เข้าถึงสินค้าที่ `is_active=false`
    ผ่าน `?id=<uuid>` ตรงๆ ได้เดิมทีเงียบๆ (แค่ query ตรงด้วย id เฉยๆ ไม่เคยเช็ค `is_active`) — `products-
    render.js`'s `fetchProductById()` เพิ่ม `.eq('is_active', true)` เข้าไปในเงื่อนไข query (pattern เดียว
    กับที่ `fetchProducts()`/`fetchCategories()` ใช้อยู่แล้ว) ทำให้สินค้าที่ปิดใช้งานคืนค่า `null` แล้ว
    `product-detail.html` **redirect ไป `404.html`** (path relative — เหตุผลเดียวกับ `page-render.js`'s
    404 redirect ดูหัวข้อ "หน้า 404" ใต้ "ระบบจัดการเพจ") แทนที่จะโชว์ข้อความ "ไม่พบสินค้านี้" แบบเดิมที่ยัง
    อยู่ในหน้าเดียวกันเฉยๆ — ครอบคลุมทั้ง 3 เคส (ไม่มี `?id=`, id ผิด/ไม่มีอยู่จริง, สินค้าถูกปิดใช้งาน) ด้วย
    เงื่อนไขเดียว (`!product`) — ฝั่ง CMS (`cms/products.js`) ไม่กระทบ เพราะโหลด/แก้ไขสินค้าจาก `items` array
    ที่ query แยกไม่ผ่าน `fetchProductById()` เลย (แอดมินยังเปิด/แก้ไขสินค้าที่ปิดอยู่ได้ตามปกติ)
...
- **หน้าเว็บหลักที่ต่อสายแล้ว**:
  ...
  - `news-detail.html` — **หน้าใหม่ที่สร้างขึ้น** route ผ่าน `?id=<uuid>` แทนที่ 3 หน้า static เดิม
    ...
    ⚠️🔧 **แก้บั๊ก 2026-08-04**: เข้าถึงบทความที่ `is_active=false` ผ่าน `?id=<uuid>` ตรงๆ ได้เดิมทีเงียบๆ
    เหมือนกับที่เจอใน `product-detail.html` (`news-render.js`'s `fetchArticleById()` เพิ่ม
    `.eq('is_active', true)` ให้แล้ว → **redirect ไป `404.html`** เมื่อไม่พบ ครอบคลุมทั้งไม่มี `?id=`/id ผิด/
    บทความถูกปิดใช้งาน — ฝั่ง CMS ไม่กระทบเหตุผลเดียวกับสินค้า) ดูรายละเอียดเต็มที่หัวข้อ "ระบบจัดการสินค้า"
    ด้านบน (pattern เดียวกัน)
...
- **ขอบเขต 5 แบบ**:
  ...
  2. **`online_shop.html`/`newsroom.html` แบบเสริม (additive)** — container เดียวกันวางไว้ก่อน `<footer>`
     เท่านั้น ไม่แตะ section เดิมที่ทำงานอยู่แล้ว (Online Shop carousel/สินค้า, Newsroom filter tiles+รายการ
     บทความ — ตั้งใจคงไว้เป็น static/functional เดิม ไม่ย้ายเข้าระบบนี้ เพราะมี JS ผูกอยู่จริงและไม่ใช่
     component แบบ image+text ทั่วไป) — container นี้ตั้ง `data-additive="true"` ไว้ (เพิ่ม 2026-07-31 สำหรับ
     online_shop, เพิ่ม 2026-08-04 สำหรับ newsroom) ให้ยกเว้นจากการ redirect ไปหน้า 404 อัตโนมัติ เพราะเนื้อหา
     จริงของหน้าเป็นอิสระจาก Page Management อยู่แล้ว ดูหัวข้อ "หน้า 404 (Page Not Found)" ด้านล่าง —
     **⚠️ `newsroom.html` เพิ่มเข้ามาเพื่อให้เพิ่ม KV Banner ให้หน้านี้ได้เท่านั้น** (ผู้ใช้ขอ — ดูหัวข้อย่อย
     "หน้า newsroom.html" ท้ายหัวข้อนี้สำหรับรายละเอียดเต็ม รวมถึงบั๊กที่พบว่า KV banner ของหน้านี้เคยถูกสร้าง
     ไว้แล้วแต่ไม่เคยแสดงผลได้เลยตั้งแต่แรก)
...
### หน้า newsroom.html — เพิ่ม Page Management แบบเสริม (additive) เพื่อรองรับ KV Banner (2026-08-04)

**บริบท**: ผู้ใช้ขอให้ newsroom.html รองรับ KV Banner — เดิม `contact.html`/`newsroom.html` ทั้งคู่ถูกกันออก
จาก Page Management โดยตั้งใจตั้งแต่แรก (มีฟอร์ม/เป็นหน้า listing แบบ dynamic อยู่แล้ว) แต่พอตรวจสอบพบว่า
**KV Banner render ผ่าน "static proxy" `page_sections` mechanism เท่านั้น** (ดูหัวข้อ "static proxy section"
ด้านบน) ซึ่งต้องมี `#pageSectionsContainer`+`page-render.js` ในหน้านั้นถึงจะมีที่ให้ proxy section render ลงไป
ได้ — เพจที่ไม่เคยมี Page Management เลยแบบ newsroom.html จึง **เพิ่ม KV Banner ไม่ได้เลยแม้แต่น้อย ทั้งที่หน้า
"จัดการแบนเนอร์" (cms/banners.html) ไม่เคยบล็อกไม่ให้เลือกหน้านี้จาก dropdown** (เพราะ page-picker ดึงจาก
`menu_items` ที่ active ล้วนๆ ไม่เช็คว่าเพจนั้นมี Page Management จริงหรือเปล่า) — **ระหว่างตรวจสอบพบเพิ่มว่า
แอดมินเคยเพิ่ม KV banner ให้ newsroom.html ไว้แล้วจริงหนึ่งอัน** (`banners.id=8b444cf8-2770-4c5a-ae1d-
0cee23d20e99`, `is_active=true`) **แต่ไม่เคยแสดงผลได้เลยตั้งแต่สร้าง** เพราะ `ensureKvProxySection()` หา
`pages` row ของ `page_key='newsroom'` ไม่เจอ (ไม่เคย seed) เลย silently return โดยไม่สร้าง proxy row ให้เลย

- **แก้โดยเพิ่ม newsroom.html เข้า "ขอบเขต 5 แบบ" ข้อ 2 (additive) แบบเดียวกับ `online_shop.html` เป๊ะๆ**:
  เพิ่ม `<script src="page-render.js">` (วางหลัง `banner-render.js` ก่อน `cart.js` ตาม pattern เดิม) +
  DOMPurify CDN (จำเป็นสำหรับ sanitize custom-html — newsroom.html ไม่เคยโหลดมาก่อน) +
  `<div id="pageSectionsContainer" data-page-key="newsroom" data-additive="true"></div>` วางก่อน `<footer>`
  (ตำแหน่งเดียวกับ online_shop.html) — เนื้อหาจริงของหน้า (filter tiles + รายการบทความ ผูกกับ news-render.js)
  ไม่ถูกแตะเลย
- **`cms/seed-newsroom-page.sql`** (ไฟล์ใหม่ **ยังไม่ได้รัน**) — insert แถวใน `pages` (`page_key='newsroom'`,
  ผูก `menu_item_id` ของเมนู "ข่าวสารและกิจกรรม" จริง) **พร้อม backfill proxy `page_sections` row ให้ KV
  banner ที่มีอยู่แล้วนั้นในตัวเดียวกันเลย** (query หา `banners` ที่ `page_url='newsroom.html' and
  section='kv'` แล้ว insert `anchor_id='kv-banner-<banner id>'` ให้ตรงกับ pattern ที่
  `ensureKvProxySection()` สร้างปกติทุกประการ) — ปลอดภัยรันซ้ำได้ (ลบ `pages` row เดิม cascade ก่อน insert
  ใหม่เสมอ)
- **ไม่ได้ทำ**: ไม่ได้เช็คว่า page-picker ของ `cms/banners.html` ควรกรองเฉพาะเพจที่มี Page Management จริงๆ
  ก่อนให้เลือกหรือเปล่า (ต้นเหตุที่แท้จริงที่ทำให้สร้าง KV banner ของหน้าที่ยังไม่รองรับได้แบบเงียบๆ ตั้งแต่แรก)
  — นอกขอบเขตที่ผู้ใช้ขอรอบนี้ (แค่ "ให้ newsroom รองรับ") ถ้าจะเพิ่มหน้าใหม่แบบเดียวกันอีกในอนาคตต้องเจอ
  ปัญหาเดิมซ้ำถ้าลืมทำตามขั้นตอนนี้ (เพิ่ม container/script + seed `pages` row) ก่อนเสมอ
- **ทดสอบ**: ตรวจสอบโค้ดเทียบกับ `online_shop.html` เป๊ะๆ เท่านั้น (ไม่มี session login จริงให้ทดสอบ end-to-end
  ว่า KV banner ที่มีอยู่จริงแล้วแสดงผลถูกต้องหลังรัน SQL — ต้องรัน `cms/seed-newsroom-page.sql` แล้วเปิด
  newsroom.html จริงเพื่อยืนยันอีกที)
...
- **⚠️🔧 Guard**: ตอนสร้างเพจ standalone ใน `cms/pages.js` เช็คว่า slug ที่ตั้งชนกับไฟล์ `.html` จริงที่มี
  อยู่แล้วในโปรเจกต์ไหม (เช่น พิมพ์ slug "career" ทั้งที่มี `career.html` จริงอยู่แล้ว) — ถ้าชนจะ block พร้อม
  ข้อความเตือน เพราะไม่งั้นเพจนั้นจะเปิดไม่ได้เลยแบบเงียบๆ (Netlify serve ไฟล์จริงเสมอ ไม่มีวัน rewrite ไปที่
  `promo.html?slug=...` ให้)
  > ⚠️🔧 **แก้บั๊กสำคัญ 2026-08-04**: `checkFileExists()` เดิมเช็คด้วย `fetch(HEAD)` จริงไปที่ root
  > (`res.ok` = ไฟล์มีอยู่จริง) — **พังหลัง deploy `_redirects`** เพราะกฎ rewrite `/*.html
  > /promo.html?slug=:splat 200` ใน `_redirects` จับ path `.html` ที่ไม่มีไฟล์จริงตรงกัน**ทุกอัน**แล้วตอบ
  > สถานะ **200** เสมอ (ไม่ใช่ 404 — เพราะมันถูก rewrite ไป `promo.html` สำเร็จ ไม่ใช่ path ที่หาไม่เจอ) ทำให้
  > `fetch` แยกแยะ "ไฟล์จริง" กับ "ไม่มีไฟล์เลย" ผ่าน HTTP status ไม่ได้อีกต่อไป — **ทุก slug ที่พิมพ์เข้ามาจะ
  > ดูเหมือนชนไฟล์จริงตลอด บล็อกการสร้างเพจ standalone ใหม่ทั้งหมดแบบถาวร** (พบจริงจากรายงานผู้ใช้: สร้างเพจ
  > slug "test4" ที่ไม่มีไฟล์ `test4.html` อยู่จริงเลย แต่ระบบขึ้น error ว่าชนไฟล์) — เป็นบั๊กคนละสาเหตุกับ
  > ข้อจำกัดเดิมที่เคยบันทึกไว้เรื่อง `file://` protocol (`fetch` throw แทนที่จะได้ 404) แต่ผลลัพธ์เหมือนกัน
  > คือ block ทุกครั้ง — **แก้โดยเลิกใช้ fetch เช็คไปเลย** เปลี่ยนเป็นเช็คกับ **ลิสต์คงที่**
  > `KNOWN_HTML_FILES` (array ชื่อไฟล์ `.html` จริงทั้งหมดที่ root ของโปรเจกต์ ซิงค์กับผลลัพธ์จริงของ
  > `find . -maxdepth 1 -iname "*.html"` ณ ตอนแก้ — 27 ไฟล์) `checkFileExists()` กลายเป็นฟังก์ชัน sync ธรรมดา
  > เช็ค `indexOf` (case-insensitive) ไม่พึ่งเครือข่ายเลย ทำงานถูกต้องแน่นอนไม่ว่าจะ deploy `_redirects`
  > แล้วหรือยัง — ⚠️ **ต้องอัปเดต `KNOWN_HTML_FILES` ในนี้ทุกครั้งที่เพิ่ม/ลบ/เปลี่ยนชื่อไฟล์ `.html` จริงที่
  > root ของโปรเจกต์** (จุดที่ 3 ที่ต้องซิงค์ ต่อจาก href ใน static HTML + `url` column ใน `menu_items` ที่
  > เตือนไว้แล้วด้านบน) — **`cms/menu.js`'s `checkFileExists()` (ใช้ตอน auto-สร้างเพจให้เมนูใหม่) ยังเป็น
  > `fetch(HEAD)` แบบเดิมอยู่ ไม่ได้แก้รอบนี้** เพราะผลกระทบต่างกัน: ถ้าเช็คผิดเป็น "มีไฟล์อยู่จริง" เสมอ จะ
  > แค่ไม่ swap URL เมนูไปเป็น `promo.html?slug=...` ให้ — แต่กฎ rewrite ใน `_redirects` (หลัง deploy) จะจับ
  > URL ที่ไม่มีไฟล์จริงให้เองอยู่ดี ไม่ทำให้เมนูเปิดไม่ได้เหมือนที่ `cms/pages.js` เจอ (ยืนยันด้วยการตรวจโค้ด
  > เท่านั้น ไม่ได้ทดสอบ end-to-end) — ถ้าเจอปัญหาจริงกับจุดนี้ในอนาคตให้ใช้ pattern `KNOWN_HTML_FILES`
  > เดียวกันนี้แก้ได้เลย
...
- **ทดสอบแล้ว**: `currentPromoSlug()` ทดสอบเป็น pure function ผ่าน browser ยืนยันครบทุก case (query string
  เดิม, path แบบใหม่, ไม่มี slug เลย, ทั้งสองแบบพร้อมกันให้ query string ชนะ) — `checkFileExists()` เวอร์ชัน
  ใหม่ (2026-08-04) ทดสอบผ่าน `deno run` ตรงๆ นอก browser (ฟังก์ชัน sync ล้วน ไม่ต้อง mock DOM/network):
  slug "career" → `true` (ชนไฟล์จริง, ถูกต้อง), slug "test4"/"summer-sale-2026" → `false` (ไม่ชน, ถูกต้อง —
  นี่คือเคสที่บั๊กเดิมรายงานผิด), case-insensitive ("CAREER.html" → `true`) ถูกต้อง — และยืนยัน
  `KNOWN_HTML_FILES` ตรงกับผลลัพธ์จริงของ `find` ที่ root เป๊ะทั้ง 27 ไฟล์ (diff ไม่มีผลต่าง)
...
## Dropzone อัปโหลดรูปภาพกลาง — ใช้ร่วมกันทุกจุดที่มี image upload ใน CMS (2026-07-31, อัปเดต 2026-08-04)

> **อัปเดต 2026-08-04 — พรีวิวรูปย้ายเข้าไปอยู่ใน dropzone เอง พร้อมปุ่ม "✕" มุมขวาบนสำหรับลบ** (ตาม mockup
> ที่ผู้ใช้ส่งมา) — เดิม dropzone แสดงกล่องอัปโหลด (ไอคอน+ข้อความ) อยู่ตลอดเวลาไม่ว่าจะมีรูปแล้วหรือไม่ ส่วนรูป
> ที่อัปโหลดแล้วไปแสดงแยกเป็น thumbnail เล็ก 64×64px ข้างๆ (`<img class="cms-image-field__preview">` นอก
> dropzone) — ตอนนี้เปลี่ยนเป็น: **มีรูปแล้ว dropzone จะโชว์รูปนั้นเต็มพื้นที่แทนไอคอน/ข้อความอัปโหลดไปเลย**
> (class `.has-image` toggle ไอคอน/หัวข้อ/hint ให้ซ่อน, โชว์ `<img class="cms-dropzone__preview">` แทน) พร้อม
> ปุ่ม "✕" ลอยมุมขวาบน (`<button class="cms-dropzone__remove">`) กดแล้วเคลียร์ค่ากลับไป state เดิมที่อัปโหลด
> ใหม่ได้ทันที — **คลิกที่ตัวรูปเองยังเปิด file picker เพื่อเปลี่ยนรูปใหม่ได้ปกติ** (dropzone's click handler
> เดิมไม่ได้ถูกแตะ ครอบคลุมทั้งรูปด้วยอัตโนมัติ) เฉพาะปุ่ม "✕" เท่านั้นที่ `e.stopPropagation()` กันไม่ให้ trigger
> file picker ซ้อนตอนกดลบ — ลบ `<img class="cms-image-field__preview">` แยกข้างนอกทิ้งทั้ง 5 จุด (ไม่ใช้แล้ว)
> ปรับใน `cms/upload.js`'s `cmsBindImageUpload()` จุดเดียว ครอบคลุมทั้ง 5 จุด single-image field อัตโนมัติ (เมนู/
> บทความ/Banner TH+EN/พื้นหลัง section) — **`cms/page-editor.html`'s ปุ่มแยก "🗑 ลบพื้นหลัง" (`#removeBgImageBtn`,
> เดิมอยู่นอก dropzone ทำหน้าที่เดียวกัน) ถูกลบทิ้งไปด้วยเพราะซ้ำซ้อนกับปุ่ม "✕" ใหม่แล้ว** (ลบทั้ง markup ใน
> `page-editor.html` และ event listener ใน `page-editor.js`)
>
> **Multi-image gallery** (สินค้าสูงสุด 5 ภาพ/`products.js`, section เพจสูงสุด 4 ภาพ/`page-editor.js`) ก็ปรับ
> ให้ใช้ปุ่ม "✕" ลอยมุมขวาบนแบบเดียวกัน (`.cms-product-image-slot__remove`, ตำแหน่งเล็กกว่า 20px แทน 24px ให้
> เข้ากับ slot ขนาด 96×96px) **แทนที่ปุ่ม 🗑 เดิมที่เคยอยู่ในแถบล่าง** (`.cms-product-image-slot__actions`) —
> ของ `products.js`: แถบล่างเหลือแค่ปุ่ม ★ "ตั้งเป็นภาพปก" (เฉพาะรูปที่ไม่ใช่ index 0) เท่านั้น เพราะ remove ย้าย
> ไปเป็นปุ่ม ✕ แยกแล้ว — ของ `page-editor.js`: **ไม่มี concept ภาพปก เลยลบแถบล่าง `.cms-product-image-slot__actions`
> ทิ้งไปทั้งกล่อง** (เหลือแค่ badge ตัวเลขลำดับมุมซ้ายบนเดิม + ปุ่ม ✕ มุมขวาบนใหม่) — ปุ่ม "+" เพิ่มรูปท้ายกริด
> (`.cms-product-image-add`) และ hint ข้อความใต้กริด **ไม่ได้แตะ** ยังเป็น pattern เดิมจากรอบ 2026-07-31
>
> **CSS ใหม่**: `.cms-dropzone__preview`/`.cms-dropzone__remove` (+ `.cms-dropzone.has-image` modifier) และ
> `.cms-product-image-slot__remove` ใน `cms/style.css` — ทั้งคู่ reuse ดีไซน์เดียวกัน (วงกลมขาวโปร่งแสงเล็กๆ มุม
> ขวาบน, hover เปลี่ยนสีแดง) ต่างกันแค่ขนาด
>
> **hint คำแนะนำใต้แต่ละ dropzone ไม่ได้แก้ข้อความเลย** (ผู้ใช้ระบุให้ใช้ตามที่มีอยู่ปัจจุบันของแต่ละหน้า — รวมถึง
> ขนาดรูปแนะนำที่ต่างกันต่อ field ที่เพิ่งปรับไปก่อนหน้านี้ในวันเดียวกัน เช่น Hero/KV Banner ดูหัวข้อ "ระบบ Banner")
> ยกเว้นแก้ typo เล็กน้อยที่เจอระหว่างทาง ("รรองรับ" → "รองรับ" ใน `news-articles.html`) ซึ่งไม่ใช่การเปลี่ยนเนื้อหา
>
> **ทดสอบ**: ยืนยันผ่าน browser จริงด้วยการ inject markup+CSS เข้าไปในหน้า CMS ที่โหลดอยู่จริง (ไม่ได้ login เข้า
> ระบบจริงเพราะไม่มี credential — ทดสอบเฉพาะ CSS/markup rendering ไม่ใช่ full user flow): ทั้ง single-dropzone
> filled-state (รูป+ปุ่ม✕ถูกตำแหน่งถูกต้อง, ซ่อนไอคอน/ข้อความเดิมถูกต้อง) และ multi-image slot (ปุ่ม✕มุมขวาบน +
> badge ภาพปก/ปุ่ม★ ไม่ชนกัน) ถูกต้องตรงตาม mockup — ระหว่างทดสอบเจอว่า preview browser tool cache
> stylesheet เก่าค้างในบาง tab (ไม่ใช่บั๊กจริงในโค้ด) ต้อง force-reload stylesheet ด้วย cache-busting query
> string เพื่อยืนยันแยกจากปัญหาจริง
>
> ก่อนหน้า (2026-07-31): ปรับทุกจุดที่มีฟังก์ชันอัปโหลดรูปภาพในทั้ง CMS (7 จุด — ดูรายชื่อด้านล่าง) ให้เป็นดีไซน์เดียวกันตามตัวอย่างที่
...
> ⚠️ อัปเดต 2026-08-06 — **เฉพาะ `cms/page-editor.html`'s เนื้อหา section เท่านั้น** (ผู้ใช้ขอเจาะจงจุดนี้ —
> ไม่ได้แตะ `products.js`/`news-articles.js`) เพิ่มปุ่ม "</>" ข้าง label "เนื้อหา (TH)"/"(EN)" สลับไปแก้โค้ด
> HTML ดิบตรงๆ ผ่าน `<textarea>` แทน Quill WYSIWYG ได้ (คนละกลไกกับ layout `custom-html` เดิมที่มีช่องโค้ด
> HTML ของตัวเองอยู่แล้วสำหรับทั้ง section — ปุ่มนี้ใหม่สำหรับ layout ปกติที่ใช้ Quill) กดซ้ำเพื่อกลับ WYSIWYG
> จะ parse โค้ดกลับเข้า Quill ผ่าน `quill.clipboard.dangerouslyPasteHTML()` เพื่อแสดงตัวอย่างในกล่อง WYSIWYG
> เท่านั้น — implement อยู่ใน `cms/page-editor.js` เอง (ไม่ได้ทำเป็นฟังก์ชันกลางใน `cms/upload.js` เพราะใช้จุด
> เดียว) — CSS: `.cms-rich-editor__label-row`/`__source-toggle`/`.cms-rich-editor__source` —
> ⚠️🔧 **แก้บั๊ก 2026-08-06 (พบทันทีหลังใช้งานจริง)**: `dangerouslyPasteHTML()` จำกัดแค่ format ที่ Quill
> รู้จัก (`CMS_QUILL_FORMATS`) ตัดทิ้งทุกอย่างนอกเหนือจากนั้น (`<div>`/`<table>`/`<iframe>`/class/data-*/
> inline style อื่นนอกจาก color) — ค่าที่จะ **save จริง** (`draft[key]`) ตอนนี้ตั้งจาก `sourceTextarea.value`
> ตรงๆ เสมอตอนกดกลับ WYSIWYG (ไม่ผ่าน Quill เลย) ส่วน `text-change` handler เช็ค `source==='user'` ก่อน sync
> กันไม่ให้ `dangerouslyPasteHTML()`'s `source='api'` ทับ draft ด้วยเวอร์ชันที่ถูกตัดทิ้งไปแล้ว — กล่อง WYSIWYG
> อาจแสดงผลไม่ตรงกับที่ save จริง 100% ถ้าโค้ดมี tag ที่ Quill ไม่รู้จัก (ยอมรับเป็น trade-off — live preview
> ด้านล่างซึ่งไม่ผ่าน Quill เลยถูกต้องเสมอ) ดูรายละเอียดเต็มที่หัวข้อ "ระบบจัดการเพจ" ด้านบน
...
0f. **รัน `cms/seed-newsroom-page.sql` ใน Supabase SQL Editor** (งานใหม่ล่าสุด 2026-08-04) — จำเป็นก่อนที่ KV
    Banner ของ newsroom.html (ทั้งอันที่มีอยู่แล้วตอนนี้ และอันใหม่ที่จะเพิ่มในอนาคต) จะแสดงผลได้จริง — โค้ด
