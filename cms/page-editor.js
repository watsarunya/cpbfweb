...
(function () {
  ...
  function renderImagesGrid() {
    ...
    sectionImages.forEach(function (url, i) {
      ...
      // ปุ่ม "✕" มุมขวาบน — ลบภาพนี้ (แพทเทิร์นเดียวกับ .cms-dropzone__remove ของ single-image dropzone)
      var removeBtn = document.createElement('button');
      removeBtn.type = 'button';
      removeBtn.className = 'cms-product-image-slot__remove';
      removeBtn.setAttribute('aria-label', 'ลบภาพนี้');
  ...
  function buildQuickEditFields(item, draft, frame) {
    ...
    [{ key: 'body_th', suffix: '(TH)' }, { key: 'body_en', suffix: '(EN)' }].forEach(function (cfg) {
      ...
      // แถวหัว label + ปุ่ม "</>" สลับไปแก้โค้ด HTML ดิบโดยตรง (นอกเหนือจากแก้ผ่าน WYSIWYG ตามปกติ)
      var labelRow = document.createElement('div');
      labelRow.className = 'cms-rich-editor__label-row';
      var label = document.createElement('label');
      label.textContent = 'เนื้อหา ' + cfg.suffix;
      var sourceToggleBtn = document.createElement('button');
      sourceToggleBtn.type = 'button';
      sourceToggleBtn.className = 'cms-rich-editor__source-toggle';
      sourceToggleBtn.title = 'แก้ไขโค้ด HTML โดยตรง';
      sourceToggleBtn.textContent = '</>';
      labelRow.appendChild(label);
      labelRow.appendChild(sourceToggleBtn);
      ...
      // textarea โค้ด HTML ดิบ — ซ่อนไว้ก่อน สลับแสดงคู่กับ editorHost (ไม่แสดงพร้อมกันทั้งคู่)
      var sourceTextarea = document.createElement('textarea');
      sourceTextarea.className = 'cms-rich-editor__source';
      sourceTextarea.hidden = true;
      sourceTextarea.spellcheck = false;

      field.appendChild(labelRow);
      field.appendChild(editorHost);
      field.appendChild(sourceTextarea);
      ...
      // ⚠️🔧 แก้บั๊ก 2026-08-06: เช็ค source ('user' vs 'api') ก่อน sync draft[cfg.key] — Quill จำกัดฟอร์แมตไว้
      // แค่ CMS_QUILL_FORMATS (header/bold/italic/underline/color/align/list/link/image) เท่านั้น การเรียก
      // quill.clipboard.dangerouslyPasteHTML() (ใช้ตอนสลับกลับจาก source mode ด้านล่าง) จะ parse โค้ด HTML
      // ผ่าน Quill's Delta model ซึ่ง**ทิ้งทุกอย่างที่ไม่ใช่ format ที่รู้จักไปเลย** (ทดสอบแล้ว: <div>/<table>/
      // <iframe>/class/data-* attribute/inline style นอกเหนือจาก color หายหมด เหลือแค่ <p>/<span> ธรรมดา) —
      // เดิม handler นี้ sync draft[cfg.key] จาก quill.root.innerHTML ทุกครั้งที่ text-change ไม่ว่า source
      // ไหน ทำให้พอกด "</>" กลับมา WYSIWYG โค้ดที่ผู้ใช้เขียนไว้ (เก็บอยู่ใน draft[cfg.key] แล้วถูกต้องจาก
      // source-mode's input handler ด้านล่าง) ถูกทับด้วยเวอร์ชันที่ผ่าน Quill มาแล้ว (เสียหาย/ถูกตัดทิ้งไปแล้ว)
      // ทันที — dangerouslyPasteHTML() ส่ง source='api' เสมอ (ไม่ใช่ 'user') ต่างจากตอนผู้ใช้พิมพ์/กดปุ่ม
      // toolbar เองจริงๆ ซึ่งเป็น 'user' — เช็คแยกตรงนี้ทำให้ sync เฉพาะตอนแก้ผ่าน WYSIWYG จริงๆ เท่านั้น
      ...
      // สลับ WYSIWYG ↔ โค้ด HTML ดิบ — สลับกลับเข้า WYSIWYG จะตั้ง draft[cfg.key] จากโค้ดดิบที่พิมพ์ไว้ตรงๆ
      // ก่อนเสมอ (ค่าที่จะถูก save จริง ไม่ผ่าน Quill เลย) แล้วค่อยเรียก dangerouslyPasteHTML() แค่เพื่อให้กล่อง
      // WYSIWYG แสดงตัวอย่างเท่าที่ Quill พอแสดงได้ (source='api' จาก text-change ด้านบนจะไม่ทับ draft ซ้ำ) —
      // ถ้าโค้ดมี tag/attribute ที่ Quill ไม่รู้จัก กล่อง WYSIWYG จะแสดงผลไม่ตรงกับที่ save จริง 100% (ยอมรับ
      // เป็น trade-off เดียวกับ editor แบบ hybrid ทั่วไป) แต่ live preview ด้านล่าง (ไม่ผ่าน Quill เลย) และค่า
      // ที่ save จริงจะถูกต้องเสมอ — ถ้าผู้ใช้พิมพ์/แก้ต่อใน WYSIWYG หลังจากนี้ (source='user') จะ sync ทับด้วย
      // เวอร์ชันที่ Quill เข้าใจแทน (คาดหวังได้ตามปกติของ editor แบบผสม)
      var isSourceMode = false;
      sourceToggleBtn.addEventListener('click', function () {
        isSourceMode = !isSourceMode;
        sourceToggleBtn.classList.toggle('is-active', isSourceMode);
        if (isSourceMode) {
          sourceTextarea.value = draft[cfg.key] || '';
          ...
          sourceTextarea.hidden = true;
          draft[cfg.key] = sourceTextarea.value;
          quill.clipboard.dangerouslyPasteHTML(sourceTextarea.value || '');
          refresh();
        }
      });

      // พิมพ์โค้ดดิบระหว่างอยู่ใน source mode ก็ sync เข้า draft + refresh preview สดๆ เหมือน WYSIWYG
