...
(function () {
  ...
  var quillTh = null;
  var quillEn = null;
  var savingToggleIds = new Set(); // id ของสินค้าที่กำลังส่ง toggle is_active ค้างอยู่ (กันกดซ้ำ + โชว์สถานะ "กำลังบันทึก")
  ...
  function buildRow(item) {
    ...
    var isSaving = savingToggleIds.has(item.id);
  ...
  // ⚠️🔧 แก้บั๊ก 2026-08-04: เดิมใช้ window.cmsSupabase.from().update() ธรรมดา ซึ่งพิสูจน์แล้วว่าถ้าผู้ใช้กด
  // toggle หลายอันติดกันเร็วๆ แล้วรีเฟรชก่อน request ถึงเซิร์ฟเวอร์ (ยืนยันจริงผ่าน updated_at ใน DB — ค่าที่
  // ตั้งจากการกดรอบหลังไม่ถูกบันทึกเลยสักแถว) browser จะยกเลิก request ที่ค้างอยู่ทั้งหมดทันทีตอน unload
  // — เปลี่ยนมาใช้ window.cmsUpdateWithKeepalive() (cms/app.js) แทน ซึ่งส่ง request ด้วย {keepalive:true}
  // ทำให้ request รอดจากการ navigate/refresh/ปิดแท็บได้จริง (แก้ที่ต้นเหตุ ไม่ใช่แค่ทำให้กดซ้ำยากขึ้น) —
  // ยังคง disable toggle + toast ไว้เหมือนเดิมเพื่อ UX ที่ชัดเจนว่ากำลังบันทึกอยู่
  async function toggleActive(item) {
    var next = !item.is_active;
    item.is_active = next;
    savingToggleIds.add(item.id);
    render();

    var error = null;
    try {
      // ⚠️ fallback เผื่อ cms/app.js เวอร์ชันเก่า (cache ค้าง) ยังไม่มีฟังก์ชันนี้ — กันโยน TypeError ที่จะ
      // ทำให้ savingToggleIds.delete() ด้านล่างไม่ถูกเรียก ปุ่มจะค้างเป็น disabled ถาวร (เจอจริงตามที่ผู้ใช้
      // รายงาน — finally ด้านล่างกันเคสนี้ไว้อีกชั้นด้วย ไม่ว่าจะโยน error อะไรก็ตาม)
      var updateFn = window.cmsUpdateWithKeepalive || function (table, id, patch) {
        return window.cmsSupabase.from(table).update(patch).eq('id', id);
      };
      var result = await updateFn(TABLE, item.id, { is_active: next });
      error = result.error;
    } catch (err) {
      error = { message: err.message || String(err) };
    } finally {
      savingToggleIds.delete(item.id);
    }
    ...
    if (error) {
      ...
      render();
      window.cmsToast(next ? 'เปิดใช้งานสินค้าเรียบร้อยแล้ว' : 'ปิดใช้งานสินค้าเรียบร้อยแล้ว', 'success');
    }
  }
