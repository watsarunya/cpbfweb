/* cms/product-categories.js — หมวดหมู่สินค้า: CRUD, drag-and-drop reorder, search */
(function () {
  ...
  async function deleteItem(item) {
    ...
    var msg = count
      ? 'ลบ "' + item.name_th + '"? มีสินค้าผูกอยู่กับหมวดหมู่นี้ ' + count + ' รายการ (สินค้าจะกลายเป็นไม่มีหมวดหมู่ ไม่ถูกลบ) การกระทำนี้ย้อนกลับไม่ได้'
  ...
  async function submitForm(e) {
    ...
    var nameTh = document.getElementById('fieldCategoryNameTh').value.trim();
    var nameEn = document.getElementById('fieldCategoryNameEn').value.trim();
    var slug = document.getElementById('fieldCategorySlug').value.trim().toLowerCase();

    if (!nameTh || !nameEn || !slug) {
      formError.textContent = 'กรุณากรอกชื่อหมวดหมู่ทั้งภาษาไทย/อังกฤษ และ slug';
      return;
    }
    if (!/^[a-z0-9-]+$/.test(slug)) {
