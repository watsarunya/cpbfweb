...
(function () {
  ...
  var savingToggleIds = new Set(); // id ของบทความที่กำลังส่ง toggle is_active ค้างอยู่ (กันกดซ้ำ + โชว์สถานะ "กำลังบันทึก")
  ...
  function buildRow(item) {
    ...
    toggle.disabled = savingToggleIds.has(item.id);
  ...
  // ⚠️🔧 แก้บั๊ก 2026-08-04 — เหตุผลเดียวกับที่แก้ใน cms/products.js's toggleActive() (ดูคอมเมนต์ที่นั่น):
  // เปลี่ยนจาก window.cmsSupabase.from().update() เป็น window.cmsUpdateWithKeepalive() (cms/app.js) เพื่อกัน
  // request ถูกยกเลิกถ้าผู้ใช้รีเฟรช/ปิดแท็บก่อน request ถึงเซิร์ฟเวอร์ — ยังคง disable toggle + toast ไว้
  async function toggleActive(item) {
    ...
    var error = null;
    try {
      // ⚠️ fallback เผื่อ cms/app.js เวอร์ชันเก่า (cache ค้าง) ยังไม่มีฟังก์ชันนี้ — เหตุผลเดียวกับที่แก้ใน
      // cms/products.js's toggleActive() (ดูคอมเมนต์ที่นั่น) — finally ด้านล่างกันปุ่มค้าง disabled ถาวร
    ...
    if (error) {
      ...
      render();
      window.cmsToast(next ? 'เปิดใช้งานบทความเรียบร้อยแล้ว' : 'ปิดใช้งานบทความเรียบร้อยแล้ว', 'success');
    }
  }
