...
(function () {
  ...
  async function loadItems() {
    ...
    items = data || [];
    await Promise.all([loadMenuItems(), loadSectionCounts()]);
    render();
  }
  ...
  // รายชื่อไฟล์ .html จริงทั้งหมดที่ root ของโปรเจกต์ (ตรวจสอบด้วยลิสต์คงที่แทนการ fetch HEAD จริง — เดิมเคย
  // ใช้ fetch แล้วเช็ค res.ok แต่ใช้งานไม่ได้เลยหลัง deploy _redirects เพราะกฎ rewrite `/*.html
  // /promo.html?slug=:splat 200` จับ path .html ที่ไม่มีไฟล์จริงตรงกันทั้งหมดแล้วตอบ 200 เสมอ (ไม่ใช่ 404)
  // ทำให้ fetch แยกแยะ "ไฟล์จริง" กับ "ไม่มีไฟล์เลย" ผ่าน HTTP status ไม่ได้อีกต่อไป — ทุก slug ที่พิมพ์เข้ามา
  // จะดูเหมือนชนไฟล์จริงตลอด บล็อกการสร้างเพจ standalone ใหม่ทั้งหมด) — ⚠️ ต้องอัปเดตลิสต์นี้ทุกครั้งที่เพิ่ม/
  // ลบ/เปลี่ยนชื่อไฟล์ .html จริงที่ root ของโปรเจกต์ (จุดเดียวกับที่ CLAUDE.md เตือนไว้เรื่อง href/menu_items)
  var KNOWN_HTML_FILES = [
  ...
  function checkFileExists(filename) {
    return KNOWN_HTML_FILES.indexOf(String(filename).toLowerCase()) !== -1;
  }
