...
(function () {
  ...
  var SECTION_LABELS = {
    hero: { label: 'Hero Banner', hint: 'แบนเนอร์ชิ้นบนสุดของหน้าเพจ ขนาดอ้างอิง 1440×600px (หน้าแรก) หรือ 1440×450px (หน้าเพจอื่นๆ) — จัดการแยกตามแต่ละหน้าเพจ เลือกหน้าด้านล่างเพื่อดู/แก้ไขแบนเนอร์ของหน้านั้น' },
    kv: { label: 'KV Banner', hint: 'แบนเนอร์เสริมภายในเนื้อหาเพจ ขนาดอ้างอิง 1440×360px ทุกหน้า — จัดการแยกตามหน้าเพจเหมือน Hero Banner และจัดลำดับเทียบกับ Section อื่นๆ ของเพจนั้นได้ผ่านหน้า "จัดการ Section" (cms/page-editor.html)' },
  };

  // ขนาดรูปที่แนะนำ ต่างกันตาม section + หน้าเพจ (Hero หน้าแรก 1440×600, Hero หน้าอื่น 1440×450, KV ทุกหน้า
  // 1440×360) — ใช้ทั้งอัปเดตข้อความใต้ dropzone ในฟอร์ม (updateImageSizeHint) และอัตราส่วนของ live preview
  // (renderHeroPreview/renderKvPreview) ให้ตรงกันเสมอ จุดเดียวที่ต้องแก้ถ้าจะเปลี่ยนขนาดแนะนำในอนาคต
  function recommendedImageSize() {
    if (currentSection === 'kv') return { w: 1440, h: 360 };
    var page = (fieldPageUrl.value || currentPage || '').trim();
    return page === 'index.html' ? { w: 1440, h: 600 } : { w: 1440, h: 450 };
  }

  function updateImageSizeHint() {
    var size = recommendedImageSize();
    // รองรับ .gif/.mp4 นอกจาก jpg/jpeg/png แล้ว (ผู้ใช้ขอ 2026-08-04) — ลิมิตขนาดไฟล์วิดีโอสูงกว่ารูปภาพ
    // (ดู MAX_VIDEO_SIZE_MB ใน cms/upload.js)
    var text = 'รองรับไฟล์ jpg, jpeg, png, gif, mp4 ขนาด ' + size.w + '×' + size.h + 'px ไม่เกิน 5 MB (วิดีโอไม่เกิน 20 MB)';
    var thHint = document.querySelector('#fieldImageThDropzone .cms-dropzone__hint');
    var enHint = document.querySelector('#fieldImageEnDropzone .cms-dropzone__hint');
    if (thHint) thHint.textContent = text;
    if (enHint) enHint.textContent = text;
  }
  ...
  // เช็คแบบเดียวกับ banner-render.js's isVideoUrl() ฝั่งเว็บหลัก/cms/upload.js's ตัวใน updatePreview() —
  // ใช้ตรวจว่ารูปที่เลือกไว้เป็นวิดีโอ (.mp4) หรือเปล่า สำหรับ preview modal นี้ (ซึ่งเดิมใช้ CSS
  // background-image ล้วนๆ ซึ่งไม่รองรับวิดีโอเลย ต้องสร้าง <video> จริงแทนเมื่อเจอไฟล์วิดีโอ)
  function isVideoUrl(url) {
    return /\.mp4(\?.*)?(#.*)?$/i.test(url || '');
  }

  function buildPreviewVideoEl(url) {
  ...
  function renderPreview() {
    ...
    if (currentSection === 'hero') {
      var heroRatio = previewItem.page_url === 'index.html' ? '1440/600' : '1440/450';
      var showcase = document.createElement('div');
      showcase.style.cssText = 'position:relative; width:100%; max-width:800px; aspect-ratio:' + heroRatio + '; border-radius:12px; overflow:hidden; background:#1a1a2e;';
      if (image && isVideoUrl(image)) {
        showcase.appendChild(buildPreviewVideoEl(image));
      } else if (image) {
      ...
      kv.style.cssText = 'position:relative; width:100%; max-width:800px; aspect-ratio:1440/360; border-radius:12px; overflow:hidden; background:#1a1a2e; display:flex; align-items:center; justify-content:' + alignToFlex(align) + '; text-align:' + align + '; padding:0 60px;';
      if (image && isVideoUrl(image)) {
        kv.appendChild(buildPreviewVideoEl(image));
      } else if (image) {
  ...
  document.addEventListener('DOMContentLoaded', async function () {
    ...
    // เปลี่ยนหน้าเพจในฟอร์ม (add/edit) — ขนาดรูปแนะนำของ Hero Banner ต่างกันระหว่างหน้าแรกกับหน้าอื่นๆ
    // ต้องอัปเดตข้อความใต้ dropzone ให้ตรงกับหน้าที่เลือกอยู่ตอนนั้นเสมอ
    fieldPageUrl.addEventListener('change', updateImageSizeHint);
